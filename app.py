    ...
    if openai_api_key:
        with st.spinner("El experto en estadística está analizando tu consulta..."):
            try:
                ...
                theory_response = response.choices[0].message.content
...
if df is not None:
    ...
    user_question = st.text_area(
        ...
        placeholder="Ej: Quiero analizar si hay diferencias en las métricas entre diferentes categorías, y cómo se relacionan con los resultados...",
        height=100,
        ...
        if openai_api_key:
            with st.spinner("OpenAI está analizando tu caso y datos..."):
                try:
                    ...
                    2. Qué pregunta de negocio/research responde
                    ...
                    analysis_recommendations = response.choices[0].message.content
                    st.success("🎯 Recomendaciones de Análisis para tus Datos:")
                    st.markdown("---")
                    st.write(analysis_recommendations)
                    st.markdown("---")
...
if df is not None:
    ...
    with tab2:  # Análisis descriptivos
        ...
        if numeric_cols:
            st.subheader("📊 Análisis Descriptivo Avanzado")
            
            if st.button("📈 Generar Reporte Descriptivo Completo"):
                with st.spinner("Generando reporte descriptivo..."):
                    try:
                        # Calcular estadísticas para todas las variables numéricas
                        descriptive_stats = {}
                        ...
                        csv = stats_df.to_csv()
                        st.download_button(
                            label="📥 Descargar reporte descriptivo (CSV)",
                            data=csv,
                            file_name="reporte_descriptivo.csv",
                            mime="text/csv"
                        )
                        
                    except Exception as e:
                        st.error(f"Error generando reporte descriptivo: {e}")
